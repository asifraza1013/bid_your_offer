...
<div class="chat-header">
    ...
    <button id="closebutton" style="width: 41px;margin-left: 79px;" class="chat-close-btn">&times;</button>
</div>
...
<div id="frame">
    ...
    <div id="contacts">
        <ul>
            ...
            <div id="token" class="d-none" data-token="{{ $chat_tokens->token }}"></div>
            @php
                $contact = $chat_tokens->chat_users->first()->user;

                if ($chat_tokens->token == $token) {
                    $current_token = $chat_tokens;
                    $current_contact = $contact;
                }

                // dd('ok');
                // dd($chat_tokens->auction);
                // dd($chat_tokens->auction->address);

                $ctype = $chat_tokens->auction_type;
                if ($ctype == 'seller-property') {
                    //  dd($chat_tokens->auction->address);
                    // dd('1');
                    $chat_title = optional($chat_tokens->auction)->address;
                    // dd('ok');
                    // $auction_link = route('view-pl', $chat_tokens->auction->id);
                    ...
                    // dd('buyer-criteria');
                    // $chat_title = $chat_tokens->title;
                    $chat_title = $chat_tokens->auction->get->property_type;
                    // dd('3');
                    // dd('3');
                    // $auction_link = route('view-pl', $chat_tokens->auction->id);
                    ...
                    // dd('5');
                    $chat_title = optional($chat_tokens->auction)->title;
            ...
            @endphp
            {{-- {{ dd('ok'); }} --}}
            <li class="contact d-none @if ($chat_tokens->token == $token) active @endif "
                data-avatar="{{ $avatar }}" data-name="{{ $contact->name }}"
                data-token="{{ $chat_tokens->token }}">
                <div class="wrap d-flex">
        ...
        @if ($current_token)
            <ul class="chat-messages">
                ...
                @foreach ($current_token->chats as $item)
                    ...
                    <li class="send12"
                        style="border-radius: 10px; background: #006e9f; display: inline-block; white-space: normal; padding: 5px; padding-top: 7px; padding-right: 16px; padding-bottom: 11px; max-width: 157px;">
                        <span class="ms-2 question_listing" style="font-size: 11px; margin-top: 10px;">
                            {{ $item->message }}
                        </span>
                    </li>
                    ...
                    <div class="align-items-end" style="display: flex; justify-content: flex-end;">
                        <li class="replies" style="background: #435f7a; display: inline-block; border-radius: 10px; padding: 5px; padding-top: 7px; padding-right: 16px; padding-bottom: 11px; max-width: 157px;">
                            <span class="ms-2" style="font-size: 11px; margin-top: 10px;">
                                {{ $item->answer }}
                            </span>
                        </li>
                    </div>
    ...
    <div class="chat-input">
        <input type="input-text" class="message-input" placeholder="Type your message">
        <button id="submit12" class="submit12" class="send-message-btn">Send</button>
    </div>
</div>
