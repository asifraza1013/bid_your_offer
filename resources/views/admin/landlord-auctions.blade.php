...
@section('content')
    <div class="card">
        <ul class="nav nav-tabs border-tab" id="top-tab" role="tablist">
            <li class="nav-item"><a href="{{ route('admin.landlord.auctions') }}?type=0"
                    class="nav-link {{ $type == '0' ? 'active' : '' }}"><i class="icofont icofont-close"></i>Pending Approval</a>
            </li>
            <li class="nav-item"><a href="{{ route('admin.landlord.auctions') }}?type=1"
                    class="nav-link {{ $type == '1' ? 'active' : '' }}"><i class="icofont icofont-check"></i>Approved</a></li>
            <li class="nav-item"><a href="{{ route('admin.landlord.auctions') }}?type=2"
            ...
            <tbody>
                ...
                @foreach ($auctions as $auction)
                    <tr>
                        ...
                        <td>{{ Carbon\Carbon::parse($auction->listing_date)->format('M j, Y') }}</td>
                        <td>
                            @if ($auction->expiration_date)
                                {{ Carbon\Carbon::parse($auction->expiration_date)->format('M j, Y') }}
                            @else
                                N/A
                            @endif
                        </td>
                        ...
                        @if ($type != 1 && $type != 2)
                            <td>
                                <a href="{{ route('admin.landlord.auction.approve', $auction->id) }}"
