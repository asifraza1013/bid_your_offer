...
@section('content')
    <div class="card">
        <ul class="nav nav-tabs border-tab" id="top-tab" role="tablist">
            <li class="nav-item"><a href="{{ route('admin.tenant.criteria.list') }}?type=0"
                    class="nav-link {{ $type == '0' ? 'active' : '' }}"><i class="icofont icofont-close"></i>Pending Approval</a>
            </li>
            <li class="nav-item"><a href="{{ route('admin.tenant.criteria.list') }}?type=1"
                    class="nav-link {{ $type == '1' ? 'active' : '' }}"><i class="icofont icofont-check"></i>Approved</a></li>
            <li class="nav-item"><a href="{{ route('admin.tenant.criteria.list') }}?type=2"
            ...
            <tbody>
                ...
                @foreach ($auctions as $auction)
                    <tr>
                        ...
                        @if ($type != 1 && $type != 2)
                            <td>
                                <a href="{{ route('admin.tenant.criteria.auction.approve', $auction->id) }}"
