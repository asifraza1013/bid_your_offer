                  ...
                  <tbody>
                    @foreach ($auctions as $auction)
                      ...
                      <tr>
                        ...
                        <td><a href="{{ route('agent.landlord.auction', @$auction->id) }}">{{ @$auction->address }}</a>
                        </td>
                        <td>{{ @$auction->county }}</td>
                        <td>{{ Carbon\Carbon::parse(@$auction->listing_date)->format('M d, Y') }}
                        </td>
                        <td>{{ Carbon\Carbon::parse(@$auction->expiration_date)->format('M d, Y') }}
                        </td>
                              ...
                              <ul class="dropdown-menu" id="modal_dropdown">
                                <li>
                                  <a class="dropdown-item" id="href_remove"
                                    href="{{ url('/renew_landloard_auction/' . $auction->id) }}">
                                ...
                                <li>
                                  @php
                                    $counter = App\Models\AgentCounterTerm::where(
                                        'agent_auction_id',
                                        @$auction->id,
                                    )->first();
                                  @endphp
                                  @if (isset($counter))
                                    <a class="dropdown-item"
                                      href="{{ route('agent.edit-counter-terms', $auction->id) }}">
                                  ...
                                  @else
                                    <a class="dropdown-item" href="{{ route('agent.counter-terms', $auction->id) }}">
                          ...
                          @else
                            <div class="dropdown">
                              ...
                              <ul class="dropdown-menu">
                                <li>
                                  <a class="dropdown-item" href="{{ route('agent.landlord.auction', @$auction->id) }}">
                                    <i class="fa-solid fa-eye" style="font-size:14px;"></i>
                                ...
                                @if (!@$auction->is_approved)
                                  <li>
                                    <a class="dropdown-item"
                                      href="{{ route('agent.landlord.auction.edit', @$auction->id) }}">
                                ...
                                <li>
                                  <a class="dropdown-item"
                                    href="{{ route('manage.bot.questions', ['landlord-property', $auction->id]) }}">
                        ...
                        <div class="modal-dialog" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title" id="exampleModalLabel">Agent's Countered Terms</h5>
                              <button type="button"
                            ...
                            <div class="modal-body">
                              <div class="row" style="flex-wrap: wrap;">
                                ...
                                @endif
                                @if (isset($counter->agentCharge))
                                  <div class="col-md-12 col-12 pt-2 removeBold"><i
                                      class="fa-regular fa-check-square"></i>
                                    <span class="fw-bold">Does the agent charge a finder’s fee to secure a property for
                                      the tenant?
                                      <span
                                        class="removeBold">{{ $counter->agentCharge == 'Yes' ? '(Yes)' : '' }}</span>
                                    </span>
                                    @if ($counter->agentCharge == 'Yes')
                                      {{ $counter->agentChargeOther }}
                                    @endif
                                  </div>
                                @endif
                                @if (isset($counter->agentFee))
                                  <div class="col-md-12 col-12 pt-2 removeBold"><i
                                      class="fa-regular fa-check-square"></i>
                                    <span class="fw-bold">Does the agent charge any fees to the tenant if the tenant
                                      cancels the Tenant Agency
                                      Agreement before the expiration date?
                                      <span class="removeBold">{{ $counter->agentFee == 'Yes' ? '(Yes)' : '' }}</span>
                                    </span>
                                    @if ($counter->agentFee == 'Yes')
                                      {{ $counter->agentFeeOther }}
                                    @endif
                                  </div>
                                @endif
                                      ...
                                      @foreach ($services as $service)
                                        <li style="font-size: 16px; margin-top:5px;">
                                          <span class="removeBold">
                                            @if ($service != 'Other-Add additional services as offered.')
                                              {{ $service }}
                                            @endif
                                            @if ($service == 'Other-Add additional services as offered.')
                                              (Other)
                                              {{ $counter->other_services }}
                                            @endif
                                          </span>
                                        </li>
                                      @endforeach
  ...
  <script>
    $(function() {
      $('.auction-type').on('change', function() {
        var val = $(this).val();
        window.location.href = '{{ route('agent.landlord.auctions') }}?type=' + val;
      });
    });
  </script>
@endpush
