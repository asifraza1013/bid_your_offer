...
@section('content')
    ...
    <div class="card">
      <div class="row">
        <div class="col-12 p-4">
          ...
          <form class="p-4 pt-0 mainform" action="{{ route('agent.tenant.agent.auction.bid.save') }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="auction_id" value="{{ @$auction->id }}">
            <div class="container">
              <h4 class="title">
                Add Bid on Hire Tenant’s Agent auction - {{ @$title }}
              </h4>
              ...
              <div class="wizard-step" data-step="4">
                @php
                  $listing_terms = [];
                  $listing_terms[] = ['target' => '', 'name' => '3 Months'];
                  $listing_terms[] = ['target' => '', 'name' => '6 Months'];
                  $listing_terms[] = ['target' => '', 'name' => '9 Months'];
                  $listing_terms[] = ['target' => '', 'name' => '12 Months'];
                  $listing_terms[] = ['target' => '.custom_listing_terms', 'name' => 'Other'];
                @endphp
                <div class="form-group">
                  <label class="fw-bold">What is the proposed timeframe outlined in the Tenant Agency Agreement?</label>
                  <select class="grid-picker" name="listing_terms" id="listing_terms" required>
                    <option value="">Select</option>
                    @foreach ($listing_terms as $service)
                      <option value="{{ $service['name'] }}" data-target="{{ $service['target'] }}" class="card flex-row"
                        style="width:calc(33.3% - 10px);" data-icon='<i class="fa-solid fa-calendar-days"></i>'>
                        {{ $service['name'] }}
                      </option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group custom_listing_terms d-none input-cover">
                  <label class="fw-bold">What is the proposed timeframe outlined in the Tenant Agency
                    Agreement?</label>
                  <input type="text" name="custom_listing_terms" class="form-control has-icon"
              ...
              <div class="wizard-step">
                @php
                  $finder_fee = [
                      [
                          'target' => '.custom_finder_fee',
                          'name' => 'Yes',
                          'icon' => '<i class="fa-regular fa-circle-check"></i>',
                      ],
                      ['target' => '', 'name' => 'No', 'icon' => '<i class="fa-regular fa-circle-xmark"></i>'],
                  ];
                @endphp
                <div class="form-group">
                  <label class="fw-bold">Does the agent charge a finder’s fee to secure a property for the tenant?</label>
                  <select class="grid-picker" name="finder_fee" id="finder_fee" required>
                    <option value="">Select</option>
                    @foreach ($finder_fee as $item)
                ...
                <div class="form-group custom_finder_fee d-none">
                  <div class="form-group">
                    <lable class="fw-bold">What is the fee that the agent charges to secure a property for the tenant?
                    </lable>
                    <input type="text" name="custom_finder_fee" id="custom_finder_lease" class="form-control has-icon"
              ...
              <div class="wizard-step">
                <div class="form-group">
                  ...
                  @endphp
                  <label class="fw-bold">Does the agent charge any fees to the tenant if the tenant cancels the Tenant
                    Agency Agreement before the expiration date?</label>
                  <select class="grid-picker" name="cancel_agreement" id="cancel_agreement" required>
                    <option value="">Select</option>
                    @foreach ($cancel_agreement as $item)
                      <option value="{{ $item['name'] }}" data-target="{{ $item['target'] }}" class="card flex-row"
                        style="width:calc(33.3% - 10px);" data-icon='{{ $item['icon'] }}'>
                        {{ $item['name'] }}
                      </option>
                ...
                <div class="form-group custom_cancels d-none">
                  <div class="form-group">
                    <label class='fw-bold'>What is the cancellation fee to the tenant? </label>
                    <input type="text" name="custom_cancels" id="custom_cancels" class="form-control has-icon"
                      data-icon="fa-solid fa-dollar-sign" required>
                  </div>
                </div>
              </div>
              ...
              <div class="wizard-step">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control has-icon" data-icon="fa-regular fa-calendar-days"
                    name="agent_license" value="{{ old('agent_license') }}" required>
                </div>
              </div>
              <div class="wizard-step">
                @php
                ...
                @endphp
                <div class="form-group">
                  <label class="fw-bold">Select the included services that the agent will provide to the tenant:</label>
                ...
                <div class="form-group other_services d-none ">
                  <label class='fw-bold'>What additional services will the agent provide to the tenant?</label>
              ...
              <div class="wizard-step">
                <div class="form-group">
                    ...
                    <input type="url" class="form-control has-icon" name="video_url" data-icon="fa-solid fa-link">
                  </div>
                  <div class="row">
              ...
              <div class="wizard-step">
                ...
                <div class="row form-group">
                  <div class="col-md-6">
                    ...
                    <input type="text" class="form-control has-icon" data-icon="fa-solid fa-handshake"
                      name="brokerage" id="brokerage" required value="{{ Auth::user()->brokerage }}">
                  </div>
                  <div class="col-md-6">
                    <label class="fw-bold">Real Estate License #: <span class="text-danger"></span></label>
                    <input type="text" class="form-control has-icon" data-icon="fa-solid fa-id-card"
                      name="license_no" required value="{{ Auth::user()->license_no }}" />
                  </div>
                </div>
                <div class="row form-group">
                  <div class="col-md-6">
                    <label class="fw-bold">NAR Member ID (NRDS ID): <span class="text-danger"></span></label>
                    <input type="text" class="form-control has-icon" data-icon="fa-solid fa-id-card-clip"
                      name="mls_id" required value="{{ Auth::user()->mls_id }}" />
                  </div>
                </div>
              </div>
              <div class="d-flex justify-content-between form-group mt-4">
                ...
                <div>
                  <button type="button" id="nextBtn" class="wizard-step-next btn btn-success btn-lg text-600"
  ...
  <template class="city_temp link_temp">
    <div class="form-group">
  ...
  <template class="city_temp promo_temp">
    <div class="form-group row">
      <div class="">
        <input type="file" name="promo[]" class="form-control">
      </div>
    </div>
  </template>
@endsection
@push('scripts')
  ...
  <script>
    function add_link() {
      var addLink = $('.link_temp').html();
      $('.addBtn').before(addLink);
      initialize();
    }

    function add_promotional() {
      var addPromo = $('.promo_temp').html();
      $('.addPromoBtn').before(addPromo);
      initialize();
    }
    $(function() {
