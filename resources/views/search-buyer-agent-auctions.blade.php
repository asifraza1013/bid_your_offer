          ...
          <div class="col-sm-12 col-md-3 col-lg-3">
            <div class="mb-3">
              <select name="property_type" onchange="javascript:$('.search-form').submit();" class="form-select">
                ...
                @foreach (\App\Models\PropertyType::all() as $pt)
            ...
            <div class="card" style="overflow: hidden;">
              <div class="card-body pb-2 pt-2">
                ...
                <div class="qr-code" style="width: 50px; height:50px; position: absolute; top:0; right:0;">
                  {{ qr_code(route('buyer.view-auction', @$auction->id), 150) }}
                </div>
                ...
                <div class="d-flex" style="justify-content:space-between">
                  <p><svg xmlns="http://www.w3.org/2000/svg" class="clock" fill="none" viewBox="0 0 24 24"
                      stroke="currentColor">
                      ...
                      class="timer-{{ @$auction->id }} badge bg-info">{{ round(@$auction->get->auction_length_days) <= 0 ? 'No Time Limit' : $diff . 'd ' . $start->diff($end)->format('%H:%I:%S') }}</b>
                  </p>
                  <p>
                    @foreach ($buyers as $buyer)
                      @if ($auction->id == $buyer->id)
                        @if ($buyer->is_sold)
                          <b class="timer-{{ @$auction->id }} badge bg-info">Agent Hired</b>
                        @else
                          <b class="timer-{{ @$auction->id }} badge bg-info">Active</b>
                        @endif
                      @endif
                    @endforeach
...
@push('scripts')
  ...
  @foreach ($pAuctions as $pa)
    @php
      $start = $carbon::now();
      $end = $carbon::parse($pa->created_at)->addDays($pa->get->auction_length_days);
      $diff = $end->diffInDays($start);
    @endphp
    {{--
...
--}}
    @if (round($pa->get->auction_length_days) > 0)
      @php
