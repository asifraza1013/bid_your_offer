  ...
  <div class="container p-4">
    ...
    <div class="card m-4">
      ...
      <div class="card-body">
        ...
        <form class="p-4 pt-0 mainform" action="{{ route('agent.update-counter-terms', $counter->id) }}" method="POST"
          enctype="multipart/form-data">
          @csrf
          <input type="hidden" name="agentId" value="{{ $counter->id }}">
          @php
            $yes_or_nos = [
          ...
          <div class="wizard-step">
            <div class="form-group">
              <label class="fw-bold" for="timeframe">What is the proposed timeframe outlined in the Tenant Agency
                Agreement?</label>
              <input type="text" name="timeframe" class="form-control has-icon hide_arrow"
                data-icon="fa-solid fa-qrcode" value="{{ $counter->timeframe }}" required>
            </div>
          </div>
          <div class="wizard-step">
            @php
              $agentCharge = [
                  ['name' => 'Yes', 'target' => '.agentChargeYes', 'icon' => 'fa-regular fa-circle-check'],
                  ['name' => 'No', 'target' => '', 'icon' => 'fa-regular fa-circle-xmark'],
              ];
            @endphp
            <div class="form-group">
              <label class="fw-bold">Does the agent charge a finder’s fee to secure a property for the tenant?</label>
              <select class="grid-picker" name="agentCharge" required>
                <option value="">Select</option>
                @foreach ($agentCharge as $item)
                  <option value="{{ $item['name'] }}" {{ $counter->agentCharge == $item['name'] ? 'selected' : '' }}
          ...
          <div class="wizard-step">
            @php
              $agentFee = [
                  ['name' => 'Yes', 'target' => '.agentFeeYes', 'icon' => 'fa-regular fa-circle-check'],
                  ['name' => 'No', 'target' => '', 'icon' => 'fa-regular fa-circle-xmark'],
              ];
            @endphp
            <div class="form-group">
              <label class="fw-bold">Does the agent charge any fees to the tenant if the tenant cancels the Tenant Agency
                Agreement before the expiration date?</label>
              <select class="grid-picker" name="agentFee" required>
                <option value="">Select</option>
                @foreach ($agentFee as $item)
                  <option value="{{ $item['name'] }}" {{ $counter->agentFee == $item['name'] ? 'selected' : '' }}
                    data-target="{{ $item['target'] }}" class="card flex-row" style="width:calc(33.3% - 10px);"
                    data-icon='<i class="{{ $item['icon'] }}"></i>'>
                    {{ $item['name'] }}
                  </option>
          ...
          <div class="wizard-step">
            ...
            <div class="form-group other_services {{ $counter->other_services ? 'd-block' : 'd-none' }}">
              <label class="fw-bold">What additional services will the agent provide to the Seller?</label>
              <input type="text" name="other_services" value="{{ $counter->other_services }}" id="other_services"
                class="form-control has-icon" data-icon="fa-solid fa-hand-point-right" required />
            </div>
          </div>
          <div class="wizard-step">
            <div class="form-group">
              <label class="fw-bold">Additional Details or Countered Terms: </label>
              <textarea class="form-control" name="additionalDetails" rows="5" required>{{ $counter->additionalDetails }}</textarea>
            </div>
            <div class="form-group">
              <div class="container">
