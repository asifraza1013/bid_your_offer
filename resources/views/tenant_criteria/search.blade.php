                    ...
                    <div class="col-sm-12 col-md-3 col-lg-3">
                        <div class="mb-3">
                            <select name="bedrooms" onchange="javascript:$('.search-form').submit();" class="form-select form-control">
                                ...
                                @foreach (\App\Models\Bedroom::all() as $bedroom)
                                    <option value="{{ $bedroom->name }}" {{selected($bedroom->name, request()->bedrooms)}}>{{ $bedroom->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-3 col-lg-3">
                        <div class="mb-3">
                            <select name="bathrooms" onchange="javascript:$('.search-form').submit();" class="form-select">
                                ...
                                @foreach (\App\Models\Bathroom::all() as $bathroom)
                                    <option value="{{ $bathroom->name }}" {{selected($bathroom->name, request()->bathrooms)}}>{{ $bathroom->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="mb-3">
                            <select name="property_type" onchange="javascript:$('.search-form').submit();" class="form-select">
                                ...
                                @foreach (\App\Models\PropertyType::all() as $pt)
                                    <option value="{{ $pt->name }}" {{selected($pt->name, request()->property_type)}}>{{ $pt->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="card" style="overflow: hidden;">
                            <div class="card-body pb-2 pt-2">
                                ...
                                <div class="qr-code" style="width: 50px; height:50px; position: absolute; top:0; right:0;">
                                    {{qr_code(route('tenant.criteria.auction.view',@$auction->id), 150)}}
                                </div>
                            ...
                            <div class="card-footer bg-light">
                                <div class="row">
                                    ...
                                    <div class="col-6 right text-end">
                                        <b>${{ number_format(@$auction->get->monthly_price) }}</b>
                                    </div>
                                </div>
                            </div>
