                                ...
                                @forelse ($auctions as $pa)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <ol>
                                                @foreach (json_decode($pa->services) as $service_id)
                                                    @if($serviceModel::find($service_id))
                                                        <li style="font-size:14px;">{{$serviceModel::find($service_id)->name}}</li>
                                                    @endif
                                                @endforeach
                                            </ol>
                                            @endif
                                        ...
                                        <td>{{$pa->address}}</td>
                                        <td>{{$pa->created_at->format('M d, Y h:i A')}}</td>
                                        <td>
                                            <a href="{{route('seller.service.auction.edit', $pa->id)}}" class="btn btn-success btn-sm">Edit</a>
                                            <a href="{{route('seller.service.auction.view', $pa->id)}}" class="btn btn-info btn-sm">View</a>
                                        </td>
                                    </tr>
                                    <!-- Section 2  -->
                                    {{-- <div class="card mb-3 p-3">
                                        <div class="row g-0">
                                            <div class="col-md-12">
                                                <div class="card-body">
                                                    <a href="{{ route('agent.service.auction.view', $pa->id) }}"
                                    ...
                                    <div class="d-flex justify-content-between buttons flex-wrap">
                                        ...
                                        @if ($type != '4' && $type != '3')
                                            <button>
                                                <a href="{{ route('agent.service.auction.edit', $pa->id) }}">
    ...
    <script>
        $(function() {
            $('.auction-type').on('change', function() {
                var val = $(this).val();
                window.location.href = '{{ route('seller.service.auction.list') }}?type=' + val;
            });
        });
    </script>
@endpush
