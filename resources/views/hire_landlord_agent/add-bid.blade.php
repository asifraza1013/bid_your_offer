    ...
    <div class="card m-4">
      ...
      <div class="card-body">
        ...
        <form class="p-4 pt-0 mainform" action="{{ route('landlord.agent.auction.bid.save') }}" method="POST"
              ...
              <select class="grid-picker" name="listing_terms" id="listing_terms" style="justify-content: flex-start;"
                ...
                @foreach ($listing_terms as $item)
                  <option value="{{ $item['name'] }}" data-target="{{ $item['target'] }}" class="card flex-row"
                    style="width:calc(25% - 10px);" data-icon='<i class="fa-solid fa-calendar-days"></i>'>
                    {{ $item['name'] }}
                  </option>
                @endforeach
              </select>
          ...
          <div class="wizard-step">
            <div class="form-group">
              <label class="fw-bold" for="offering_price">What is the total commission the agent will charge the
                landlord?
              </label>
            ...
            {{-- <div class="form-group">
              ...
              <input type="text" name="agentCommission" id="" class="form-control has-icon hide_arrow"
                data-icon="fa-solid fa-ruler-combined" data-msg-required="" required>
            </div>
            <div class="form-group">
              @php
          ...
          <div class="wizard-step">
            @php
              $agentCharges = [['target' => '.agentYes', 'name' => 'Yes', 'icon' => 'fa-regular fa-check-circle'], ['target' => '.agentNo', 'name' => 'No', 'icon' => 'fa-regular fa-circle-xmark']];
            @endphp
            <div class="row align-items-end mt-4">
              <div class="col-md-12">
                <label class="fw-bold" for="heated_sqft">Does the agent charge any fees if the landlord terminates the listing agreement before the expiration date? </label>
                <div class="select2-parent">
                  ...
                  <div class="form-group agentYes d-none">
                    <label class="fw-bold" for="heated_sqft"> What is the cancellation fee if the landlord terminates the agent's listing agreement before the expiration date? </label>
                    <input type="text" name="custom_agent_charges" id="total_acreage"
                      class="form-control has-icon hide_arrow" data-icon="fa-solid fa-dollar-sign" required>
                  </div>
                </div>
          ...
          <div class="wizard-step">
            <div class="form-group">
              ...
              <input type="text" class="form-control has-icon" name="licensed" value="{{ old('licensed') }}"
          ...
          <div class="wizard-step">
            @php
              ...
              if ($auction->get->property_type == 'Residential Property') {
                  $services_data = [
                    ['name' => 'Conduct a thorough rental market analysis (RMA) to determine the property\'s value and pricing strategy.', 'target' => ''],
                    ['name' => 'List the property on the Bid Your Offer platform.', 'target' => ''],
                    ['name' => 'List the property on the MLS.', 'target' => ''],
                    ['name' => 'List the property on major real estate websites, such as Zillow, Trulia, Realtor.com, Homes.com, Homesnap, Hotpads, and others, to increase visibility and exposure.', 'target' => ''],
                    ...
                    ['name' => 'Conduct real estate email marketing campaigns that lead to the property listing.', 'target' => ''],
                    ...
                    ['name' => 'Provide a floor plan of the property to highlight its layout and spatial configuration.', 'target' => ''],
                    ['name' => 'Provide virtual staging to enhance the property\'s visual appeal and attract potential tenants.', 'target' => ''],
                    ['name' => 'Host an Open House.', 'target' => ''],
                    ['name' => 'Send email alerts to tenants searching for properties that match the property\'s criteria the moment the property is listed directly through the MLS.', 'target' => ''],
                    ['name' => 'Conduct property showings and viewings for interested tenants.', 'target' => ''],
                    ['name' => 'Provide regular updates on market activity, showings, and feedback from potential tenants.', 'target' => ''],
                    ...
                    ['name' => 'Assist in negotiating residential lease terms, including rental price, lease duration, and any additional clauses or provisions.', 'target' => ''],
                    ['name' => 'Assist in drafting residential lease agreements and required addendums/disclosures.', 'target' => ''],
                    ['name' => 'Assist with lease renewal negotiations and adjustments to rental terms.', 'target' => ''],
                    ['name' => 'Assist with tenant move-in and move-out inspections.', 'target' => ''],
                    ['name' => 'Coordinate property maintenance and repairs through trusted contractors and vendors.', 'target' => ''],
                    ['name' => 'Handle tenant inquiries, maintenance requests, and resolve any issues that may arise during the tenancy.', 'target' => ''],
                    ['name' => 'Coordinate or assist in the move-in or move-out process for tenants.', 'target' => ''],
            ...
            <div class="form-group other_services @if (@$auction->get->other_services == '' || @$auction->get->other_services == 'null') d-none @endif ">
              ...
              <input type="text" name="other_services" id="other_services" data-icon="fa-solid fa-ruler-combined"
                value="{{ @$auction->get->other_services }}" class="form-control has-icon">
            </div>
          </div>
          <div class="wizard-step">
            <div class="form-group row">
              <label class="fw-bold">Virtual Listing Presentation (Link):
                <input type="url" class="form-control has-icon" name="video_file" data-icon="fa-solid fa-link">
            </div>

            <div class="row">
                <div class="col-6">
                  <label class="fw-bold mt-1">Virtual Listing Presentation (Upload):</label>
                  <div class="videoBox ">
                    <div class="video bgImg"></div>
                    <div class="form-group videoDiv">
                      <input type="file" class="fileuploader" name="video_file" style="display: none;"
                        accept="video/*">
